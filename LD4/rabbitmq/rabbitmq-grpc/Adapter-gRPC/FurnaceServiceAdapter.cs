...
public class FurnaceServiceAdapter
{
	...
	public FurnaceServiceAdapter()
	{
		while(true){
			try{
				log.Info("Connecting to gRPC server");
				...
				log.Warn(e, "Failed to connect to gRPC server, retrying");
				Thread.Sleep(2000);
			}
		}
	...
	private void OnMessageReceived(IModel channel, BasicDeliverEventArgs msgIn)
	{
		try
		{
			...
			switch( request.Action )
			{
				...
				{
					//make the call
					var result = furnace!.GetUniqueId(new Empty()).Value;
				...
				{
					//make the call
					var result = furnace!.GetFurnaceState(new Empty()).Value;
