...
/// </summary>
class FurnaceClient : SRabbitMQ.IFurnaceService
{
	/// <summary>
		...
		if( resultDataExtractor != null )
		{
			...
			consumer.Received +=
				(channel, delivery) => {
					...
					{
						var response = JsonConvert.DeserializeObject<SRabbitMQ.RPCMessage>(Encoding.UTF8.GetString(delivery.Body.ToArray()));
	...
	public SRabbitMQ.FurnaceState GetFurnaceState() 
	{
		var result =
			Call(
				nameof(GetFurnaceState),
				null,
				(data) => JsonConvert.DeserializeAnonymousType(data, new {Value = SRabbitMQ.FurnaceState.Pouring}).Value
			);
		return result;
	}
	...
	public SRabbitMQ.CycleAttemptResult MeltGlass(SRabbitMQ.ClientDesc client)
	{
		var result =
			Call(
				nameof(MeltGlass),
				() => JsonConvert.SerializeObject(client),
				(data) => JsonConvert.DeserializeObject<SRabbitMQ.CycleAttemptResult>(data)
			);
		return result;
	}
}